...
const recipes = {
  ...
  getById: (recipeId) =>
    NodeApi.get(`http://localhost:3000/recipe/${recipeId}`),
  ...
  delete: (recipeId) =>
    NodeApi.delete(`http://localhost:3000/recipe/${recipeId}`),
  getRandom: (options) =>
    NodeApi.get('http://localhost:3000/recipes/random', options),
}
